# LLMWiki Codex Prompts: ingest workflow
...
```text
请在 Windows PowerShell 上为当前 LLMWiki 项目安装并配置 Codex custom prompts workflow。
...
4. 创建三个 prompt 文件：
   - %USERPROFILE%\.codex\prompts\ingest.md
   - %USERPROFILE%\.codex\prompts\check-raw.md
   - %USERPROFILE%\.codex\prompts\queue.txt
5. 根据当前仓库根目录的 custom-prompts.md，提取并写入三个 prompt：
   - ingest.md：单篇论文 ingest workflow。
   - check-raw.md：扫描 raw/papers/ 与 wiki/papers/ 并生成 ingest.txt 的 workflow。
   - queue.txt：读取 ingest.txt 并顺序执行队列的 workflow。
6. 修改 PowerShell Profile，也就是 $PROFILE，添加以下 functions：
   - ingest：读取 %USERPROFILE%\.codex\prompts\ingest.md，并把参数作为 raw/papers/$File 传给 Codex。
   - check：当参数为 raw 时读取 %USERPROFILE%\.codex\prompts\check-raw.md 并调用 Codex。
   - queue：读取 %USERPROFILE%\.codex\prompts\queue.txt 并调用 Codex。
